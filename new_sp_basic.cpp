...
void SerialParallelRuleOfMixturesLaw::CalculateMaterialResponsePK2(ConstitutiveLaw::Parameters& rValues)
{
    ...
    if (r_flags.Is(ConstitutiveLaw::COMPUTE_STRESS)) {
        ...
        Vector fiber_stress_vector(voigt_size), matrix_stress_vector(voigt_size);
        ...
        Vector parallel_strain(voigt_size);
        noalias(parallel_strain) = prod(parallel_projector, Vector(prod(trans(parallel_projector), r_strain_vector)));
        if (mIsPrestressed) {
            parallel_strain[0] += rValues.GetElementGeometry().GetValue(SERIAL_PARALLEL_IMPOSED_STRAIN);
        }

        values_fiber.SetStrainVector(parallel_strain);
        values_matrix.SetStrainVector(r_strain_vector);

        // Integrate Stress of the matrix
        values_matrix.SetMaterialProperties(r_props_matrix_cl);
        mpMatrixConstitutiveLaw->CalculateMaterialResponse(values_matrix, ConstitutiveLaw::StressMeasure_PK2);
        noalias(matrix_stress_vector) = values_matrix.GetStressVector();

        // Integrate Stress of the fiber
        values_fiber.SetMaterialProperties(r_props_fiber_cl);
        mpFiberConstitutiveLaw->CalculateMaterialResponse(values_fiber, ConstitutiveLaw::StressMeasure_PK2);
        noalias(fiber_stress_vector) = prod(parallel_projector, Vector(prod(trans(parallel_projector), values_fiber.GetStressVector())));

        Vector parallel_matrix_stress(voigt_size);
        noalias(parallel_matrix_stress) = prod(parallel_projector, Vector(prod(trans(parallel_projector), matrix_stress_vector)));

        Vector& r_integrated_stress_vector = rValues.GetStressVector();
        noalias(r_integrated_stress_vector) = mFiberVolumetricParticipation * fiber_stress_vector + (1.0 - mFiberVolumetricParticipation)*parallel_matrix_stress + (matrix_stress_vector-parallel_matrix_stress);
		// end new things
...
)
{
    ...
    int iteration = 0, max_iterations = 1500;
    ...
    if (iteration > max_iterations) {
        #pragma omp critical
        {
            KRATOS_WATCH("Maximum number of interations inside the Serial-Parallel algorithm")
            std::cout << "matrix: " + std::to_string(MathUtils<double>::Norm(rMatrixStressVector)) << std::endl;
            std::cout << "fiber: " + std::to_string(MathUtils<double>::Norm(rFiberStressVector)) << std::endl;
            std::cout << "ERROR of: " + std::to_string(MathUtils<double>::Norm(stress_residual)) << std::endl;
            std::cout << "Element: " << (rValues.GetElementGeometry().Id()) << std::endl;
            KRATOS_ERROR_IF(iteration > max_iterations) << "Maximum number of interations inside the Serial-Parallel algorithm" << std::endl;
        }
    }
}
    ...
    double& rValue)
{
    if (rThisVariable == UNIAXIAL_STRESS_MATRIX) {
        auto matrix_values = rParameterValues;
        const auto &props = rParameterValues.GetMaterialProperties();
        const auto it_cl_begin = props.GetSubProperties().begin();
        const auto r_props_matrix_cl = *(it_cl_begin);
        matrix_values.SetMaterialProperties(r_props_matrix_cl);
        mpMatrixConstitutiveLaw->CalculateValue(matrix_values, UNIAXIAL_STRESS, rValue);
    } else if (rThisVariable == UNIAXIAL_STRESS_FIBER) {
        auto fiber_values = rParameterValues;
        const auto &props = rParameterValues.GetMaterialProperties();
        const auto it_cl_begin = props.GetSubProperties().begin();
        const auto r_props_fiber_cl  = *(it_cl_begin + 1);
        fiber_values.SetMaterialProperties(r_props_fiber_cl);
        mpFiberConstitutiveLaw->CalculateValue(fiber_values, UNIAXIAL_STRESS, rValue);
    }
    return rValue;
}
