                    ...
                    if (GetKeepSystemConstantDuringIterations() == false)
                    {
                        ...
                        if (damped) {
                            ...
                            if (new_residual > old_residual) {
                                ...
                                while (new_residual > old_residual && iteration < 1e10) {
                                    TSparseSpace::SetToZero(rb);
                                    // we update lambda
                                    lambda = 1.0 - new_residual / (old_residual + new_residual);
                                    if (lambda < 0.0)
                                        lambda = 0.01;
                                    else if (lambda > 1.0)
                                        lambda = 0.99;
                                    TSparseSpace::InplaceMult(rDx, lambda);
                                    UpdateDatabase(rA, rDx, rb, BaseType::MoveMeshFlag());
                                    p_builder_and_solver->BuildRHS(p_scheme, r_model_part, rb);
                                    CalculateResidualNorm(r_model_part, new_residual, r_dof_set, rb);
                                    iteration++;
                                ...
                                std::cout << "dx factored by " << std::to_string(lambda) << std::endl;
